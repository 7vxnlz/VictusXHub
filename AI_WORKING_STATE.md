...
- Unavailable: CPU package temperature, V1 fan RPM, keyboard state/levels, numeric charge limits, and current GPU mode. The diagnostic-only, exact-device-gated PawnIO.Modules 0.2.2 `AMDFamily17` CLI probe reads only SMN `0x00059800` as `Core (Tctl/Tdie)` and has no RyzenSMU path. Its target CPU gate is CPUID Family 19h / Model 74h / Stepping 1 (the earlier Phoenix `70h-7Fh` group was not literal Model 70h); identity uses native RSMB or the read-only BIOS registry fallback when WMI is denied. Physical identity passed, but PawnIO device open returned Win32 `5` before module load/sample collection, so the UI remains unavailable. `FanGetLevel` is raw-only and is never RPM or percent; `FanMaxGet` is inconclusive. HP performance mode is unavailable and controls remain disabled.
