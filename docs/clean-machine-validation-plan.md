...
This plan defines acceptance checks for a future `VictusXHub-HP-Diagnostic-*-win-x64-portable.zip`. It does not authorize publishing, hardware control, or developer-only WMI probes.
...
- No prior VictusXHub install, extracted preview folder, launch shortcut, or `%APPDATA%\VictusXHub` data unless the run is explicitly testing upgrade or stale-report behavior.
...
2. Confirm the package launcher is under `tools` beside `VictusXHub.exe` and start the launcher, not a bare executable.
...
- The application opens visibly as `VictusXHub Read-only Diagnostic` without an elevation prompt.
...
- All explicit probe fields in `%APPDATA%\VictusXHub\hp-capability-report.json` remain `Attempted=false`; no `hpqBIOSInt*` method is invoked.

## Report and Export Checks

- Startup creates or safely updates `%APPDATA%\VictusXHub\hp-capability-report.json` with the expected schema metadata and normal HP diagnostic mode/source.
- Missing, empty, corrupt, and older-schema reports render a safe unavailable/read-error state without a crash.
- Reload reads only the local report; Copy summary contains no raw binary data; Open report folder opens only the VictusXHub app-data folder.
- Export creates a sanitized Markdown report under `%APPDATA%\VictusXHub\Logs\Reports\` and includes the read-only/no-control disclaimer.
...
- Confirm no `VictusXHub.exe` process remains in Task Manager after each exit.
...
- Explicit VictusXHub metadata and approved icon render in Explorer, shortcut, taskbar, tray, and the window.
...
- Preserve report/export files only when needed for support; otherwise remove `%APPDATA%\VictusXHub` deliberately after confirming it contains no needed user data.
