...
| Package license/notices | Resolved for frozen candidate | Project GPL, G-Helper modified-project attribution, eight-package notice inventory, exact .NET 10.0.11 runtime notices, NvAPIWrapper treatment, and icon provenance match the frozen ZIP extraction | Preserve exact distributable bytes | [Third-Party Notices](../THIRD-PARTY-NOTICES.md), [Package license workflow](package-license-review-workflow.md) | Notice review GO; publishing remains separately blocked |
...
| Final distributable checksum | Resolved for frozen candidate | ZIP name/version, commit, 49,848,534-byte size, SHA-256, dual-tool verification, reviewer/date, and notice linkage recorded | Preserve exact bytes for clean-machine validation | [Signing workflow](signing-checksum-workflow.md), [Signing/checksum evidence plan](signing-checksum-evidence-plan.md) | GO for checksum evidence; invalidated by any byte change |
...
| Final package contents inspection | Resolved for frozen candidate | Candidate and fresh ZIP extraction both pass layout, notice, runtime, identity, forbidden-artifact, and checksum-manifest checks; all 22 files match | Use frozen ZIP unchanged for clean-machine validation | [Package inspection](preview-package-contents-inspection.md), [Release blockers](preview-release-current-blockers.md) | GO for frozen candidate |
...
Run the clean-machine validation plan against the exact frozen unsigned ZIP without rebuilding or repacking it. Preview package publish and normal fan control remain NO-GO until that evidence is complete.
