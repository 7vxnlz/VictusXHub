...
| LibreHardwareMonitor / PawnIO lineage | Normal VictusX telemetry does not initialize LibreHardwareMonitor. The developer-only `--hp-ryzen-temperature-probe` command uses the official PawnIO.Modules 0.2.2 `AMDFamily17.bin` lineage directly so that `RyzenSMU` PM-table initialization is impossible. Its exact CPU gate is Family 19h, decoded CPUID Model 74h, Stepping 1: AMD's Phoenix `70h-7Fh` family grouping was incorrectly used as a literal `70h` model before the physical target reported `74h`. Identity is read before PawnIO through native RSMB where available, then the read-only `HKLM\HARDWARE\DESCRIPTION\System\BIOS` fallback; WMI denial is not itself a failure. It opens only the already-installed PawnIO device, loads that exact signed module, calls only `ioctl_read_smn` for `THM_TCON_CUR_TMP` (`0x00059800`), and applies the current LHM `Core (Tctl/Tdie)` decode. The first physical gate pass reached PawnIO but failed closed with Win32 `5` (access denied), so no temperature sample exists and it remains outside the normal UI. |
