...
- the sixteen reviewed files under `Assets/Licenses/` recorded by `app/Assets/Licenses/LICENSE-SOURCES.md` and the runtime evidence record.
...
| `THIRD-PARTY-NOTICES.md` presence and inventory | Automated | Require the external file, byte-match it to the reviewed repository source, and fail closed if the fixed project/package/runtime/icon inventory is missing or stale. |
| License/notice files | Automated | Require `LICENSE` and every reviewed `Assets/Licenses/**` file and byte-match them to repository sources. |
