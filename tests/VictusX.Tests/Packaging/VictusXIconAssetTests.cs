...
public sealed class VictusXIconAssetTests
{
    ...
    public void ApprovedIcon_IsACompletePngBackedWindowsIcon()
    {
        ...
        string iconPath = Path.Combine(root, "app", "Assets", "VictusX.ico");
    ...
    public void ApprovedSourceAndObsoleteVariants_HaveExpectedRepositoryState()
    {
        ...
        Assert.True(File.Exists(Path.Combine(assets, "VictusX.Source.png")));
        Assert.True(File.Exists(Path.Combine(assets, "VictusX.ico")));
        Assert.False(File.Exists(Path.Combine(assets, "VictusX.Silent.ico")));
        Assert.False(File.Exists(Path.Combine(assets, "VictusX.Balanced.ico")));
        Assert.False(File.Exists(Path.Combine(assets, "VictusX.Turbo.ico")));
    }
