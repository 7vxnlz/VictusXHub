...
public sealed class HpDiagnosticReportLoaderTests
{
    ...
    public void RefreshExistingReport_UpgradesSchemaV1WithSetFanMaxExperimentalFieldsWithoutChangingInvocationFlags()
    {
        string reportPath = WriteTemporaryReport(
            "{\"ReportSchemaVersion\":1,\"ReportGeneratedBy\":\"VictusXHub\",\"ReportMode\":\"HP read-only diagnostic\",\"SystemDesignDataInvocationAttempted\":false,\"FanGetCountInvocationAttempted\":false,\"FanMaxGetInvocationAttempted\":false,\"FanGetLevelInvocationAttempted\":false}");
        try
        {
    ...
    {
        string reportPath = Path.Combine(Path.GetTempPath(), "VictusXHub-" + Guid.NewGuid() + ".json");
