...
public sealed class PreviewPackageVerificationScriptTests
{
    ...
    public void Inspector_IsReadOnlyAndRequiresAnExplicitDirectory()
    {
        string script = File.ReadAllText(Path.Combine(
            FindRepositoryRoot(), "tools", "verify-victusxhub-preview-package.ps1"));
    ...
    public void NvApiWrapperOutsidePackageRoot_IsNoGo()
    {
        ...
        Assert.Equal(1, result.ExitCode);
        Assert.Contains("FAIL nvapi-sidecar: NvAPIWrapper.dll must exist exactly once beside VictusXHub.exe", result.Output, StringComparison.Ordinal);
        Assert.EndsWith("Preview package: NO-GO", result.Output.TrimEnd(), StringComparison.Ordinal);
    }
    ...
    private static ScriptResult RunInspector(string packageDirectory)
    {
        ...
        startInfo.ArgumentList.Add(Path.Combine(repositoryRoot, "tools", "verify-victusxhub-preview-package.ps1"));
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "VictusXHub.sln")))
    ...
    private sealed class PreviewPackageFixture : IDisposable
    {
        ...
        {
            string repositoryRoot = FindRepositoryRoot();
            string directoryPath = Path.Combine(Path.GetTempPath(), "VictusXHub.Tests", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(directoryPath);
            File.WriteAllBytes(Path.Combine(directoryPath, "VictusXHub.exe"), [0x4D, 0x5A, 0x00, 0x00]);
            File.Copy(Path.Combine(repositoryRoot, "app", "app.config"), Path.Combine(directoryPath, "VictusXHub.dll.config"));
            ...
            File.Copy(
                Path.Combine(repositoryRoot, "tools", "run-victusxhub-hp-diagnostic.ps1"),
                Path.Combine(packageTools, "run-victusxhub-hp-diagnostic.ps1"));
