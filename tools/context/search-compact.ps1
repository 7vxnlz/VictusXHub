...
$preview = @(& rg @previewArguments -- $Pattern @Path 2>$null | Select-Object -First $MaxMatches)
if ($LASTEXITCODE -gt 1) { throw "rg failed with exit code $LASTEXITCODE." }
...
$result = [ordered]@{
    ...
    TopFiles = $topFiles
    Preview = $preview
    Truncated = ($files.Count -gt $MaxFiles -or $totalMatches -gt $MaxMatches)
    PreviewLimits = [ordered]@{ MaxFiles = $MaxFiles; MaxMatches = $MaxMatches; Context = $Context }
}
...
if ($result.Truncated) { Write-Output ("Preview truncated to {0} files and {1} matches. Use narrower paths/patterns or raise limits explicitly." -f $MaxFiles, $MaxMatches) }
