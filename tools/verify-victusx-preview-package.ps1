...
$requiredFiles = @(
    ...
    "tools/run-victusx-hp-diagnostic.ps1"
...
if ($missing.Count -eq 0) {
    Write-Pass "required-files" "VictusX executable, launcher, NvAPIWrapper sidecar, and all required notices are present."
} else {
...
if ($nvApiPaths.Count -eq 1 -and $nvApiPaths[0] -ceq "NvAPIWrapper.dll") {
    Write-Pass "nvapi-sidecar" "NvAPIWrapper.dll is independently replaceable beside VictusX.exe."
} else {
    Write-Fail "nvapi-sidecar" "NvAPIWrapper.dll must exist exactly once beside VictusX.exe; found: $($nvApiPaths -join ', ')."
}

$launcherRelativePath = "tools/run-victusx-hp-diagnostic.ps1"
...
if ($actualPaths.ContainsKey("VictusX.dll.config")) {
    $sourceConfigHash = Get-Sha256 (Join-Path $repositoryRoot "app/app.config")
    $candidateConfigHash = Get-Sha256 $actualPaths["VictusX.dll.config"]
    if ($sourceConfigHash -ceq $candidateConfigHash) {
        Write-Pass "application-config" "VictusX.dll.config byte-matches app/app.config."
    } else {
        Write-Fail "application-config" "VictusX.dll.config does not match app/app.config."
    }
}
...
$noticeSources = @{
    ...
    $launcherRelativePath = Join-Path $repositoryRoot "tools/run-victusx-hp-diagnostic.ps1"
}
...
if ($unexpectedExecutables.Count -eq 0) {
    Write-Pass "runtime-layout" "single VictusX executable plus the expected NvAPIWrapper sidecar; no external runtime binaries found."
} else {
    Write-Fail "runtime-layout" ("unexpected external binaries: " + ($unexpectedExecutables -join ", "))
}

if ($actualPaths.ContainsKey("VictusX.exe")) {
    $executableStream = [IO.File]::OpenRead($actualPaths["VictusX.exe"])
    try {
        $hasMzHeader = $executableStream.ReadByte() -eq 0x4D -and $executableStream.ReadByte() -eq 0x5A
    } finally {
        $executableStream.Dispose()
    }
    if ($hasMzHeader) { Write-Pass "executable-format" "VictusX.exe has a Windows executable header." }
    else { Write-Fail "executable-format" "VictusX.exe does not have a Windows executable header." }

    try {
        $versionInfo = [Diagnostics.FileVersionInfo]::GetVersionInfo($actualPaths["VictusX.exe"])
        if ([string]::IsNullOrWhiteSpace($versionInfo.ProductName)) {
            Write-Warn "executable-identity" "file metadata could not be read; verify product/version/description manually."
        } elseif ($versionInfo.ProductName -ceq "VictusX" -and $versionInfo.FileDescription -match "VictusX") {
